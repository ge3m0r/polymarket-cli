...
仓库中的 `.github/workflows/tokyo-weather-paper.yml` 默认每天 UTC 00:17（东京09:17）在 GitHub 云端运行 `signal-tokyo`。信号通过一次 CLOB `/prices` 批量请求直接抓取 YES 的实时最佳卖价，并记录实际报价时间、相对计划买点的延迟、每条策略腿的份数、每档 `best ask`、手续费和滑点成本。只有在计划时间后15分钟内抓到的快照才进入 Artifact 内的 `paper-signals/open/` 前向账本；过早或过晚的运行标记为 `SKIP_OFF_SCHEDULE`，不计入策略收益。

下一次云端运行会恢复最近的 `tokyo-weather-paper-ledger` Artifact，用 Polymarket 已结算的胜出档位结算其中的历史快照，并写入账本的 `paper-signals/settled/`。实际盈亏只使用原始买点快照，绝不在结算后重新查询价格。账本滚动保存90天，工作流对仓库保持只读，不会自动改动主分支。云端任务不依赖本机开机；选择17分运行是为了避开 GitHub Actions 整点附近的调度高峰。
